...
def index():
    ...
    if bottle.request.json[u"action"] == u"threadstatus":
        ...
        (output["threads"]["clients"]["status"],
         output["threads"]["clients"]["lastupdate"]) = taco.globals.clients.get_status()
        ...
        (output["threads"]["server"]["status"],
         output["threads"]["server"]["lastupdate"]) = taco.globals.server.get_status()
    ...
    if bottle.request.json[u"action"] == u"downloadqmove":
        ...
        with taco.globals.download_q_lock:
            logging.debug(
                "Moving File in Download Q:" + str((peer_uuid, sharedir, filename, filesize, filemod, newloc)))
            if peer_uuid in taco.globals.download_q:
                while (sharedir, filename, filesize, filemod) in taco.globals.download_q[peer_uuid]:
                    taco.globals.download_q[peer_uuid].remove((sharedir, filename, filesize, filemod))
                taco.globals.download_q[peer_uuid].insert(min(newloc, len(taco.globals.download_q[peer_uuid])),
        ...
        with taco.globals.settings_lock:
            ...
            with taco.globals.download_q_lock:
                ...
                for peer_uuid in taco.globals.settings["Peers"]:
                    try:
                        peerinfo[peer_uuid] = [taco.globals.settings["Peers"][peer_uuid]["nickname"],
        ...
        if type(bottle.request.json[u"data"]) == type({}):
            if u"sharedir" in bottle.request.json[u"data"] and u"uuid" in bottle.request.json[u"data"]:
                with taco.globals.share_listings_lock:
                    if (bottle.request.json[u"data"][u"uuid"],
                        bottle.request.json[u"data"][u"sharedir"]) in taco.globals.share_listings:
                        output = {"result": taco.globals.share_listings[
                            (bottle.request.json[u"data"][u"uuid"], bottle.request.json[u"data"][u"sharedir"])][1]}
        return json.dumps(output)
            ...
            for peer_uuid in taco.globals.settings["Peers"].keys():
                if taco.globals.settings["Peers"][peer_uuid]["enabled"]:
                    ...
                    try:
                        ...
                        nickname_status = "Unknown"
                    output[peer_uuid] = [incoming, outgoing, timediffinc, timediffout, nickname_status,
                                         taco.globals.settings["Peers"][peer_uuid]["localnick"]]
        return json.dumps(output)
        ...
        if type(bottle.request.json[u"data"]) == type([]):
            if len(bottle.request.json[u"data"]) >= 0:
                with taco.globals.settings_lock:
                    ...
                    for (hostname, port, localnick, peeruuid, clientpub, serverpub, dynamic, enabled) in \
                    bottle.request.json[u"data"]:
                        taco.globals.settings["Peers"][unicode(peeruuid)] = {"hostname": hostname, "port": int(port),
                                                                             "localnick": localnick,
                                                                             "dynamic": int(dynamic),
                                                                             "enabled": int(enabled),
                                                                             "clientkey": clientpub,
