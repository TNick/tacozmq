...
def Create_Request(command=NET_GARBAGE, data=""):
    with taco.globals.settings_lock:
        local_uuid = taco.globals.settings["Local UUID"]
    response = {
...
def Create_Reply(command=NET_GARBAGE, data=""):
    with taco.globals.settings_lock:
        local_uuid = taco.globals.settings["Local UUID"]
    reply = {
        NET_IDENT: local_uuid,
...
def Proccess_Request(packed):
    ...
    if NET_REQUEST in unpacked:
        ...
        if unpacked[NET_REQUEST] == NET_REQUEST_CERTS:
            return (
                IDENT, Reply_Certs(IDENT, unpacked[NET_DATABLOCK]))
        if unpacked[NET_REQUEST] == NET_REQUEST_CHAT:
            return (
                IDENT, Reply_Chat(IDENT, unpacked[NET_DATABLOCK]))
        if unpacked[NET_REQUEST] == NET_REQUEST_SHARE_LISTING:
            return (
                IDENT, Reply_Share_Listing(IDENT, unpacked[NET_DATABLOCK]))
        if unpacked[NET_REQUEST] == NET_REQUEST_SHARE_LISTING_RESULTS:
            return (
                IDENT, Reply_Share_Listing_Result(IDENT, unpacked[NET_DATABLOCK]))
        if unpacked[NET_REQUEST] == NET_REQUEST_GET_FILE_CHUNK:
            return (
                IDENT, Reply_Get_File_Chunk(IDENT, unpacked[NET_DATABLOCK]))
        if unpacked[NET_REQUEST] == NET_REQUEST_GIVE_FILE_CHUNK:
            return (
                IDENT, Reply_Give_File_Chunk(IDENT, unpacked[NET_DATABLOCK]))
...
def Request_Chat(chatmsg):
    output_block = Create_Request(NET_REQUEST_CHAT, [time.time(), chatmsg])
    with taco.globals.chat_log_lock:
        ...
        if len(taco.globals.chat_log) > CHAT_LOG_MAXSIZE:
...
def Reply_Chat(peer_uuid, datablock):
    ...
    with taco.globals.chat_log_lock:
        ...
        if len(taco.globals.chat_log) > CHAT_LOG_MAXSIZE:
            taco.globals.chat_log = taco.globals.chat_log[1:]
    reply = Create_Reply(NET_REPLY_CHAT, {})
    return msgpack.packb(reply)


def Request_Rollcall():
    request = Create_Request(NET_REQUEST_ROLLCALL, "")
...
def Request_Certs(peer_uuids):
    request = Create_Request(NET_REQUEST_CERTS, peer_uuids)
    return msgpack.packb(request)


def Reply_Certs(peer_uuid, datablock):
    reply = Create_Reply(NET_REPLY_CERTS, {})
    with taco.globals.settings_lock:
...
def Request_Share_Listing(peer_uuid, sharedir, share_listing_uuid):
    ...
    request = Create_Request(NET_REQUEST_SHARE_LISTING,
                             {"sharedir": sharedir, "results_uuid": share_listing_uuid})
    return msgpack.packb(request)


def Reply_Share_Listing(peer_uuid, datablock):
    reply = Create_Reply(NET_REPLY_SHARE_LISTING, 1)
    try:
...
def Reply_Share_Listing_Result(peer_uuid, datablock):
    reply = Create_Reply(NET_REPLY_SHARE_LISTING_RESULTS, 1)
    try:
        sharedir = datablock["sharedir"]
        shareuuid = datablock["results_uuid"]
        ...
        reply = Create_Reply(NET_REPLY_SHARE_LISTING_RESULTS, 0)
        return msgpack.packb(reply)
