...
def Create_Request(command=taco.constants.NET_GARBAGE, data=""):
    ...
    response = {taco.constants.NET_IDENT: localuuid, taco.constants.NET_REQUEST: command,
                taco.constants.NET_DATABLOCK: data}
    return response
...
def Create_Reply(command=taco.constants.NET_GARBAGE, data=""):
    with taco.globals.settings_lock:
        localuuid = taco.globals.settings["Local UUID"]
    reply = {taco.constants.NET_IDENT: localuuid, taco.constants.NET_REPLY:
        command, taco.constants.NET_DATABLOCK: data}
    return reply
    ...
    if taco.constants.NET_REQUEST in unpacked:
        if unpacked[taco.constants.NET_REQUEST] == taco.constants.NET_REQUEST_GIVE_FILE_CHUNK:
            if "data" in unpacked[taco.constants.NET_DATABLOCK]:
                logging.info(
                    "NET_REQUEST (FileChunk DATA): " + str(len(unpacked[taco.constants.NET_DATABLOCK]["data"])))
        else:
        ...
        if unpacked[taco.constants.NET_REQUEST] == taco.constants.NET_REQUEST_ROLLCALL:
            return (
                IDENT, Reply_Rollcall())
        if unpacked[taco.constants.NET_REQUEST] == taco.constants.NET_REQUEST_CERTS:
            return (
                IDENT, Reply_Certs(IDENT, unpacked[taco.constants.NET_DATABLOCK]))
        if unpacked[taco.constants.NET_REQUEST] == taco.constants.NET_REQUEST_CHAT:
            return (
                IDENT, Reply_Chat(IDENT, unpacked[taco.constants.NET_DATABLOCK]))
        if unpacked[taco.constants.NET_REQUEST] == taco.constants.NET_REQUEST_SHARE_LISTING:
            return (
                IDENT, Reply_Share_Listing(IDENT, unpacked[taco.constants.NET_DATABLOCK]))
        if unpacked[taco.constants.NET_REQUEST] == taco.constants.NET_REQUEST_SHARE_LISTING_RESULTS:
            return (
                IDENT, Reply_Share_Listing_Result(IDENT, unpacked[taco.constants.NET_DATABLOCK]))
        if unpacked[taco.constants.NET_REQUEST] == taco.constants.NET_REQUEST_GET_FILE_CHUNK:
            return (
                IDENT, Reply_Get_File_Chunk(IDENT, unpacked[taco.constants.NET_DATABLOCK]))
        if unpacked[taco.constants.NET_REQUEST] == taco.constants.NET_REQUEST_GIVE_FILE_CHUNK:
            return (
                IDENT, Reply_Give_File_Chunk(IDENT, unpacked[taco.constants.NET_DATABLOCK]))
...
def Process_Reply(peer_uuid, packed):
    ...
    if taco.constants.NET_REPLY in unpacked:
        ...
        if unpacked[taco.constants.NET_REPLY] == taco.constants.NET_REPLY_ROLLCALL:
            return Process_Reply_Rollcall(
                peer_uuid, unpacked[taco.constants.NET_DATABLOCK])
        if unpacked[taco.constants.NET_REPLY] == taco.constants.NET_REPLY_CERTS:
            return Process_Reply_Certs(
                peer_uuid, unpacked[taco.constants.NET_DATABLOCK])
        if unpacked[
            taco.constants.NET_REPLY] == taco.constants.NET_REPLY_GET_FILE_CHUNK:
            return Process_Reply_Get_File_Chunk(
                peer_uuid, unpacked[taco.constants.NET_DATABLOCK])
...
def Reply_Rollcall():
    ...
    with taco.globals.settings_lock:
        for peer_uuid in taco.globals.settings["Peers"].keys():
            if abs(taco.globals.clients.get_client_last_reply(
                    peer_uuid) - time.time()) < taco.constants.ROLLCALL_TIMEOUT:
                peers_i_can_talk_to.append(peer_uuid)
    reply = Create_Reply(
        taco.constants.NET_REPLY_ROLLCALL,
        [taco.globals.settings["Nickname"],
         taco.globals.settings["Local UUID"]] + peers_i_can_talk_to)
    return msgpack.packb(reply)
    ...
    with taco.globals.settings_lock:
        ...
        for peerid in unpacked[1:]:
            if taco.constants.UUID_CHECKER.match(peerid):
                if peerid not in taco.globals.settings["Peers"].keys() and peerid != taco.globals.settings[
                    "Local UUID"]:
...
def Reply_Certs(peer_uuid, datablock):
    ...
    with taco.globals.settings_lock:
        for peer_uuid in datablock:
            if peer_uuid in taco.globals.settings["Peers"]:
                reply[taco.constants.NET_DATABLOCK][peer_uuid] = [
                    taco.globals.settings["Peers"][peer_uuid]["nickname"],
                    taco.globals.settings["Peers"][peer_uuid]["hostname"],
                    taco.globals.settings["Peers"][peer_uuid]["port"],
                    taco.globals.settings["Peers"][peer_uuid][
                        "clientkey"],
                    taco.globals.settings["Peers"][peer_uuid][
                        "serverkey"],
                    taco.globals.settings["Peers"][peer_uuid]["dynamic"]
                ]
    return msgpack.packb(reply)
...
def Request_Share_Listing(peer_uuid, sharedir, share_listing_uuid):
    ...
    request = Create_Request(taco.constants.NET_REQUEST_SHARE_LISTING,
...
def Reply_Share_Listing(peer_uuid, datablock):
    ...
    with taco.globals.share_listing_requests_lock:
        if not peer_uuid in taco.globals.share_listing_requests: taco.globals.share_listing_requests[
            peer_uuid] = Queue()
...
def Request_Share_Listing_Results(sharedir, results_uuid, results):
    request = Create_Request(
        taco.constants.NET_REQUEST_SHARE_LISTING_RESULTS,
        {"sharedir": sharedir, "results_uuid": results_uuid, "results": results})
    return msgpack.packb(request)
...
def Request_Get_File_Chunk(sharedir, filename, offset, chunk_uuid):
    request = Create_Request(
        taco.constants.NET_REQUEST_GET_FILE_CHUNK,
        {"sharedir": sharedir, "filename": filename, "offset": offset, "chunk_uuid": chunk_uuid})
    return msgpack.packb(request)
...
def Reply_Get_File_Chunk(peer_uuid, datablock):
    try:
        ...
        reply = Create_Reply(
            taco.constants.NET_REPLY_GET_FILE_CHUNK, {"status": 0})
        return msgpack.packb(reply)
    taco.globals.filesys.chunk_requests_outgoing_queue.put(
        (peer_uuid, sharedir, filename, offset, chunk_uuid))
    taco.globals.filesys.sleep.set()
    reply = Create_Reply(
        taco.constants.NET_REPLY_GET_FILE_CHUNK,
...
def Request_Give_File_Chunk(data, chunk_uuid):
    request = Create_Request(
        taco.constants.NET_REQUEST_GIVE_FILE_CHUNK,
