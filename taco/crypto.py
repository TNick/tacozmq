...
def Init_Local_Crypto():
    ...
    if not os.path.isfile(os.path.abspath(os.path.normpath(
            os.path.join(privatedir, taco.constants.KEY_GENERATION_PREFIX + "-server.key")))) or not os.path.isfile(
            os.path.abspath(os.path.normpath(
                    os.path.join(privatedir, taco.constants.KEY_GENERATION_PREFIX + "-server.key_secret")))):
        server_generate = True

    client_generate = False
    if not os.path.isfile(os.path.abspath(os.path.normpath(
            os.path.join(privatedir, taco.constants.KEY_GENERATION_PREFIX + "-client.key")))) or not os.path.isfile(
            os.path.abspath(os.path.normpath(
                    os.path.join(privatedir, taco.constants.KEY_GENERATION_PREFIX + "-client.key_secret")))):
        client_generate = True

    if server_generate:
        logging.info("Server CURVE Public or Private Key Missing, Generating")
        server_public_file, server_secret_file = zmq.auth.create_certificates(workingdir,
                                                                              taco.constants.KEY_GENERATION_PREFIX + "-server")
        shutil.move(os.path.normpath(os.path.abspath(server_public_file)),
                    os.path.normpath(os.path.abspath(privatedir)))
        shutil.move(os.path.normpath(os.path.abspath(server_secret_file)),
                    os.path.normpath(os.path.abspath(privatedir)))
    if client_generate:
        logging.info("Client CURVE Public or Private Key Missing, Generating")
        client_public_file, client_secret_file = zmq.auth.create_certificates(workingdir,
                                                                              taco.constants.KEY_GENERATION_PREFIX + "-client")
        shutil.move(os.path.normpath(os.path.abspath(client_public_file)),
                    os.path.normpath(os.path.abspath(privatedir)))
        shutil.move(os.path.normpath(os.path.abspath(client_secret_file)),
                    os.path.normpath(os.path.abspath(privatedir)))
