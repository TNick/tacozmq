  ...
  $.ajax({
    ...
    success: function (data) {
      if (data["result"].length == 0) {
        $("#completedqempty").removeClass("hide");
        $("#completedfooter").addClass("hide");
      } else {
...
function Update_Download_Q() {
  if ($dontrefresh) {
    setTimeout(Update_Download_Q, 1000);
    return;
  }
  var $api_action = {
    "action": "downloadqget",
  ...
  $.ajax({
    ...
    success: function (data) {
      if ("result" in data && "peerinfo" in data && "fileinfo" in data) {
        ...
        if ($rows_added == 0) {
          $("#downloadqempty").removeClass("hide");
        };
        ...
        $(".table td span.removeiteminq").click(function (event) {
          ...
          $("#removeModal #removebutton").unbind("click").click(function () {
            ...
            var $api_action = {
              "action": "downloadqremove",
              "data": {
                "uuid": remove_q_peer_uuid,
                "sharedir": sharedir,
                "filename": filename,
                "filesize": filesize,
                "filemodtime": modtime
        ...
        $('#downloadqdiv .table').hover(function () {
          console.log("in");
          $("#hoverpaused").removeClass("hide");
          $dontrefresh = true;
        }, function () {
          console.log("out");
          $dontrefresh = false;
          $("#hoverpaused").addClass("hide");
        });
