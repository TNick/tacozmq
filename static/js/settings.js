...
function download_browse() {
  ...
  $("button[id='downloadcancelbutton']").unbind("click").click(function () {
    $("#downloadlistdiv").scrollTop(0);
    $('#downloadModal').modal('hide');
  });
  $('#downloadModal').modal();
}
...
function cert_browse() {
  ...
  $("button[id='certcancelbutton']").unbind("click").click(function () {
    $("#certloclistdiv").scrollTop(0);
    $('#certModal').modal('hide');
  });
  $('#certModal').modal();
}
...
function confirm_add_share() {
  ...
  $("button[data-type='share'][data-action='delete']").each(function () {
    if ($(this).data("name").toLowerCase() == $sharename.toLowerCase()) {
      $valid_name = false;
    }
  });
...
function Confirm_delete_share($name, $div) {
  ...
  $div.fadeOut(1000, function () {
    $(this).remove();
    if ($("div[id='share-listing'] div[class='input-group']").length == 0) {
      $("div[id='addasharebelow']").removeClass("hide");
    }
  });
  $("span[id='shares-unsaved']").removeClass("hide");
...
function save_peers() {
  ...
  $("#peer-listing .peerblock").each(function () {
    ...
    if ($host.length == 0) {
      $(this).find(".peerhost").removeClass("input-changed").addClass("input-error");
      $(this).find(".peerhostbad").removeClass("hide");
    }
    if (!port_pattern.test($port)) {
      $(this).find(".peerport").removeClass("input-changed").addClass("input-error");
      $(this).find(".peerportbad").removeClass("hide");
    }
    if (!nick_pattern.test($nick) && $nick != "") {
      $(this).find(".peerlocalnick").removeClass("input-changed").addClass("input-error");
      $(this).find(".peernickbad").removeClass("hide");
    }
    if (!uuid_pattern.test($uuid)) {
      $(this).find(".advanced-options").slideDown();
      $(this).find(".peeruuid").removeClass("input-changed").addClass("input-error");
      $(this).find(".peeruuidbad").removeClass("hide");
    }
    if (!z85_pattern.test($client)) {
      $(this).find(".advanced-options").slideDown();
      $(this).find(".peerclient").removeClass("input-changed").addClass("input-error");
      $(this).find(".peerclientbad").removeClass("hide");
    }
    if (!z85_pattern.test($server)) {
      $(this).find(".advanced-options").slideDown();
      $(this).find(".peerserver").removeClass("input-changed").addClass("input-error");
      $(this).find(".peerserverbad").removeClass("hide");
    }
...
function save() {
  ...
  if (!nick_pattern.test($nickname)) {
    $("input[id='setting-nickname']").addClass("input-error");
    $("div[id='setting-nickname-alert']").removeClass("hide");
  }
  if (!ip_pattern.test($appip)) {
    $("input[id='setting-appip']").addClass("input-error");
    $("div[id='setting-appip-alert']").removeClass("hide");
  }
  if (!ip_pattern.test($webip)) {
    $("input[id='setting-webip']").addClass("input-error");
    $("div[id='setting-webip-alert']").removeClass("hide");
  }

  if (!port_pattern.test($appport)) {
    $("input[id='setting-appport']").addClass("input-error");
    $("div[id='setting-appport-alert']").removeClass("hide");
  }
  if (!port_pattern.test($webport)) {
    $("input[id='setting-webport']").addClass("input-error");
    $("div[id='setting-webport-alert']").removeClass("hide");
  }

  if (!limit_pattern.test($down)) {
    $("input[id='setting-downlimit']").addClass("input-error");
    $("div[id='setting-down-alert']").removeClass("hide");
  }
  if (!limit_pattern.test($up)) {
    $("input[id='setting-uplimit']").addClass("input-error");
    $("div[id='setting-up-alert']").removeClass("hide");
  }
...
function Confirm_Add_Peer() {
  ...
  try {
    ...
    if ('hostname' in $json_data == false) {
      throw "No hostname field, your peer probably did not enter in a hostname before sending you the \"Quick Connect String\".";
    }
    if ('port' in $json_data == false) {
      throw "No port field,your peer probably did not enter in a port before sending you the \"Quick Connect String\".";
    }
    if ('serverkey' in $json_data == false) {
      throw "No serverkey field, your peer probably needs to regenerate his public/private keys. You should probably not be seeing this error.";
    }
    if ('clientkey' in $json_data == false) {
      throw "No clientkey field, your peer probably needs to regenerate his public/private keys. You should probably not be seeing this error.";
    }
    if (!port_pattern.test($json_data["port"])) {
      throw "Bad Port Format, your peer provided you with what appears to either be an out-of-range port, or simply not a integer.";
    }
    if (!z85_pattern.test($json_data["clientkey"])) {
      throw "Bad Clientkey Format, your peer has a bad clientkey format. You shouldn't see this error.";
    }
    if (!z85_pattern.test($json_data["serverkey"])) {
      throw "Bad Serverkey Format, your peer has a bad serverkey format. You shouldn't see this error.";
    }
    if (!uuid_pattern.test($json_data["uuid"])) {
      throw "Invalid UUID field, the UUID your peer provided doesn't appear to actually be a valid UUID.";
    }
    if ($json_data["uuid"] == $("#addpeermyuuid").val()) {
      throw "Your peer cannot have the same UUID as yourself. Don't go copying and pasting things randomly. As you see here, this won't work.";
    }
    if ($json_data["clientkey"] == $("#addpeermyclientpublic").val()) {
      throw "Cannot have a duplicate Client Key, you and your peer cannot share the same public keys.";
    }
    if ($json_data["serverkey"] == $("#addpeermyserverpublic").val()) {
      throw "Cannot have a duplicate Server Key, you and your peer cannot share the same public keys.";
    }
...
function Add_Peer() {
  $('#addPeerModal button[id="addpeercancelbutton"]').unbind("click").click(function () {
    $('#addPeerModal').modal('hide');
  });
  $('#addPeerModal button[id="addpeerconfirmbutton"]').unbind("click").click(function () {
    Confirm_Add_Peer();
  });
...
function Delete_Peer($id) {
  ...
  $("button[id='deletepeerconfirmbutton']").unbind("click").click(function () {
    Confirm_Delete_Peer($peername, $fadeoutdiv);
  });
...
function Confirm_Delete_Peer($peer, $div) {
  ...
  $div.fadeOut(1000, function () {
    $(this).remove();
    if ($("div[id='peer-listing'] .peerblock").length == 0) {
      $("div[id='addapeerbelow']").removeClass("hide");
    }
  });
  $("span[id='peers-unsaved']").removeClass("hide");
...
function set_up_peer_buttons() {
  ...
  $("button[class~='peer-static-button']").unbind("click").click(function () {
    $(this).data("status", !$(this).data("status"));
    if ($(this).find("span[class='peer-text']").html() == "Dynamic Hostname/IP") {
      $(this).find("span[class='peer-text']").html("Static Hostname/IP");
    } else {
      $(this).find("span[class='peer-text']").html("Dynamic Hostname/IP")
    }
  ...
  $(".toggle-advanced-peer-options").unbind("click").click(function () {
    ...
    if ($(this).find('.advanced-text').html() == "Hide Advanced Settings") {
      $(this).find('.advanced-text').html("Show Advanced Settings");
    } else {
      $(this).find('.advanced-text').html("Hide Advanced Settings");
    }
    $button = $(this);
...
$(document).ready(function () {
  ...
  $("button[id='browsecert']").click(function () {
    cert_browse();
  });

  $("button[id='add-peer']").click(function () {
    Add_Peer();
  });

  $("button[id='save-shares']").prop("disabled", false).click(function () {
    Save_Shares();
  });
  $("button[id='save-settings']").prop("disabled", false).click(function () {
    save();
  });
  ...
  $('#addPeerModal').on('shown.bs.modal', function () {
    $('#addpeername').focus();
    $("#addPeerModal span").popover();
  });
  ...
  $('#addPeerModal #peerneedsthis').click(function () {
    $(this).select();
  });

  $("#addpeermyhostname").on("change keyup paste", function () {
    Update_CopyPastePeerThing();
  });
  $("#addpeermyport").on("change keyup paste", function () {
    Update_CopyPastePeerThing();
  });
